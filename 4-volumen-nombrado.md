...
```
docker run -d --name server-postgres -e POSTGRES_DB=db_drupal -e POSTGRES_PASSWORD=12345 -e POSTGRES_USER=user_drupal --network net-drupal -v vol-postgres:/var/lib/postgresql/data postgres
```
...
```
docker run -d --name client-postgres --publish published=9500,target=80 -e PGADMIN_DEFAULT_PASSWORD=54321 -e PGADMIN_DEFAULT_EMAIL=admin@example.com --network net-drupal dpage/pgadmin4
```
...
```powershell
# Volúmenes recomendados para Drupal
...
```
docker run -d --name server-drupal --publish published=9700,target=80 -v vol-drupal-files:/var/www/html/sites/default/files -v vol-drupal-modules:/var/www/html/modules -v vol-drupal-profiles:/var/www/html/profiles -v vol-drupal-themes:/var/www/html/themes --network net-drupal drupal
```
